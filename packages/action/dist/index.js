...
async function run() {
    try {
        ...
        const generateHtml = (core.getInput("generate-html-report") || "false").toLowerCase() !==
            "false";
        const htmlReportPath = core.getInput("report-html-path") || "baseline-report.html";
        const generateSarif = (core.getInput("generate-sarif-report") || "false").toLowerCase() !==
            "false";
        const sarifReportPath = core.getInput("report-sarif-path") || "baseline-report.sarif";
        ...
        try {
            ...
            ({ stdout, stderr } = await pexec(`${process.execPath} ${localCli} ${scanPath} --json --exit-zero${filesArg}`));
        }
        let report;
        try {
            ...
            core.warning(`Failed to parse CLI JSON output. stdout length=${stdout?.length}; stderr length=${stderr?.length}`);
            throw e;
        }
        ...
        const top = [...nonBaseline]
            .sort((a, b) => {
            const order = (x) => x.advice === "needs-guard" ? 0 : x.advice === "guarded" ? 1 : 2;
        ...
        const tableRows = top.map((f) => {
            ...
            const advice = f.advice === "guarded"
                ? "guarded"
                : f.advice === "safe"
                    ? "safe"
                    : "needs-guard";
            const unsupported = typeof f.unsupportedPercent === "number"
        ...
        if (report.meta?.targets)
            summaryLines.push(`Targets: ${report.meta.targets.join(", ")}`);
