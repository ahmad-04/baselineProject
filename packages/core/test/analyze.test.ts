...
import { analyze, type FileRef } from "../src/index.js";

function file(path: string, content: string): FileRef {
  return { path, content };
}
...
test("guards: navigator.share is recognized as guarded", () => {
  ...
  assert.equal((share as any).advice, "guarded");
});
...
test("targets: unsupportedPercent is attached when targets provided", () => {
  ...
  assert.equal(typeof (vt as any).unsupportedPercent, "number");
});
